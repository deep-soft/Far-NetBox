...
void TTerminal::Progress(TFileOperationProgressType * AOperationProgress)
{
  if (FNesting == 0)
  {
    const TAutoNestingCounter NestingCounter(FNesting);
...
{
  const TFSProtocol OrigFSProtocol = GetSessionData()->GetFSProtocol();
  const UnicodeString PrevRemoteDirectory = GetSessionData()->GetRemoteDirectory();
  const bool PrevReadCurrentDirectoryPending = FReadCurrentDirectoryPending;
  const bool PrevReadDirectoryPending = FReadDirectoryPending;
  DebugAssert(!FSuspendTransaction);
  const bool PrevAutoReadDirectory = FAutoReadDirectory;
  ...
  const int32_t PrevExceptionOnFail = FExceptionOnFail;
  ...
  uint32_t AFlags, const UnicodeString & SpecialRetry, const UnicodeString & HelpKeyword)
{
  ...
  if (SkipToAllPossible && AOperationProgress->GetSkipToAll())
  {
    Answer = qaSkip;
  }
  else
  {
    const uint32_t Answers = qaRetry | qaAbort |
  ...
  if (Answer != qaRetry)
  {
    if ((Answer == qaAbort) && (AOperationProgress != nullptr))
    {
      AOperationProgress->SetCancel(csCancel);
    }
  ...
  uint32_t AFlags, const UnicodeString & ASpecialRetry, const UnicodeString & AHelpKeyword)
{
  ...
  {
    FileOperationLoopQuery(E, AOperationProgress, AMessage, AFlags, ASpecialRetry, AHelpKeyword);
  }
}
  ...
  const UnicodeString & AMessage, void * Param1, void * Param2)
{
  ...
  FILE_OPERATION_LOOP_END_EX(AMessage, AFlags);
...
void TTerminal::SetCurrentDirectory(const UnicodeString & AValue)
{
  DebugAssert(FFileSystem);
  if (AValue != FFileSystem->CurrentDirectory())
  {
    ChangeDirectory(AValue);
  }
}
...
{
  const bool SecureShellActive = (FSecureShell != nullptr) && FSecureShell->GetActive();
  ...
  uint32_t Answers, const UnicodeString & AHelpKeyword)
{
  ...
  else
  {
    ...
    const bool CanSkip = FLAGSET(Answers, qaSkip) && (GetOperationProgress() != nullptr);
  ...
  const UnicodeString & ASourceFullFileName, const TCopyParamType * CopyParam, int32_t Params, TFileOperationProgressType * AOperationProgress, bool Special)
{
  ...
  else if (FLAGSET(Params, cpAppend))
  {
    Result = boAppend;
           ...
           (((AOperationProgress->GetSide() == osLocal) && GetIsCapable(fcNewerOnlyUpload)) ||
            (AOperationProgress->GetSide() != osLocal)) &&
  ...
  else if (FLAGSET(Params, cpNoConfirmation) || !FConfiguration->GetConfirmOverwriting())
  {
    ...
    DebugAssert(AOperationProgress->GetBatchOverwrite() == boNo);
    Result = boAll;
  }
  else
  {
    Result = AOperationProgress->GetBatchOverwrite();
    if (!Special &&
  ...
  const UnicodeString & AFileName, const TCopyParamType * CopyParam, int32_t Params, TFileOperationProgressType * AOperationProgress)
{
  ...
  try__finally
  {
    Result = (EffectiveBatchOverwrite(AFileName, CopyParam, Params, AOperationProgress, true) != boAll);
  }
  __finally
  {
    AOperationProgress->UnlockUserSelections();
  } end_try__finally
  return Result;
}
...
uint32_t TTerminal::ConfirmFileOverwrite(
  ...
  TOperationSide Side, const TCopyParamType * CopyParam, int32_t Params, TFileOperationProgressType * AOperationProgress,
  const UnicodeString & AMessage)
{
  ...
  const bool CanAlternateResume =
    ...
    !AOperationProgress->GetAsciiTransfer();

  AOperationProgress->LockUserSelections();
  try__finally
  {
    // duplicated in TSFTPFileSystem::SFTPConfirmOverwrite
    BatchOverwrite = EffectiveBatchOverwrite(ASourceFullFileName, CopyParam, Params, AOperationProgress, true);
  ...
  if (BatchOverwrite != boNo)
  {
    switch (BatchOverwrite)
    {
      ...
      case boOlder:
        ...
        {
          const TModificationFmt Precision = base::LessDateTimePrecision(FileParams->SourcePrecision, FileParams->DestPrecision);
          const TDateTime ReducedSourceTimestamp =
            base::ReduceDateTimePrecision(FileParams->SourceTimestamp, Precision);
          const TDateTime ReducedDestTimestamp =
...
void TTerminal::EnsureNonExistence(const UnicodeString & AFileName)
{
  ...
  if ((base::UnixExtractFileDir(AFileName).IsEmpty()) &&
      base::UnixSamePath(GetCurrentDirectory(), FFiles->GetDirectory()))
  {
    const TRemoteFile * File = FFiles->FindFile(AFileName);
    if (File)
    {
...
void TTerminal::RollbackAction(TSessionAction & Action,
  TFileOperationProgressType * AOperationProgress, Exception * E)
{
...
void TTerminal::ReadCurrentDirectory()
{
  ...
  try
  {
    ...
    {
      DebugAssert(FDirectoryChangesCache != nullptr);
      const UnicodeString ACurrentDirectory = GetCurrentDirectory();
...
void TTerminal::ReadDirectory(bool ReloadOnly, bool ForceCache)
{
  ...
  if (GetSessionData()->GetCacheDirectories() && FDirectoryCache->HasFileList(GetCurrentDirectory()))
  {
    ...
    else
    {
      ...
      try__finally
      {
        LoadedFromCache = FDirectoryCache->GetFileList(GetCurrentDirectory(), FFiles.get());
      }
      __finally
      {
        DoReadDirectory(ReloadOnly);
...
TRemoteFileList * TTerminal::DoReadDirectoryListing(const UnicodeString & ADirectory, bool UseCache)
{
  ...
  try__catch
  {
    const bool Cache = UseCache && GetSessionData()->GetCacheDirectories();
...
{
  const bool Dir = (AFile != nullptr) && AFile->GetIsDirectory() && CanRecurseToDirectory(AFile);
  ...
  TProcessFileEvent && CallBackFunc, void * AParam, bool UseCache, bool IgnoreErrors)
{
  ...
  if (FileList)
  {
    try__finally
    {
      const UnicodeString Directory = base::UnixIncludeTrailingBackslash(ADirName);
...
bool TTerminal::RecycleFile(const UnicodeString & AFileName, const TRemoteFile * AFile)
{
  ...
  else
  {
...
#else
    {
      ...
      const TDateTime DateTime = Now();
      DateTime.DecodeDate(Y, M, D);
      DateTime.DecodeTime(H, N, S, MS);
      const UnicodeString dt = FORMAT("%04d%02d%02d-%02d%02d%02d", Y, M, D, H, N, S);
      // Params.FileMask = FORMAT("*-%s.*", FormatDateTime(L"yyyymmdd-hhnnss", Now()));
  ...
  const TRemoteFile * AFile, void * AParams)
{
  ...
  const int32_t Params = (AParams != nullptr) ? *(static_cast<int32_t *>(AParams)) : 0;
  const bool Recycle =
  ...
  int32_t AParams, TStrings * AFiles, TCaptureOutputEvent && OutputEvent)
{
  ...
  else
  {
    ...
    const UnicodeString Cmd =
      TRemoteCustomCommand(Data, GetCurrentDirectory(), "", FileList).
  ...
  const TRemoteFile * AFile, /*const TRemoteProperties*/ void * Properties)
{
  ...
  if (Log->GetLogging() && RProperties)
  {
    ...
    {
      uint16_t Y, M, D, H, N, S, MS;
      const TDateTime DateTime = ::UnixToDateTime(RProperties->Modification, GetSessionData()->GetDSTMode());
      DateTime.DecodeDate(Y, M, D);
      DateTime.DecodeTime(H, N, S, MS);
    ...
    {
      uint16_t Y, M, D, H, N, S, MS;
      const TDateTime DateTime = ::UnixToDateTime(RProperties->LastAccess, GetSessionData()->GetDSTMode());
      DateTime.DecodeDate(Y, M, D);
      DateTime.DecodeTime(H, N, S, MS);
      const UnicodeString dt = FORMAT("%02d.%02d.%04d %02d:%02d:%02d ", D, M, Y, H, N, S);
#if defined(__BORLANDC__)
  ...
  const TRemoteFile * AFile, /*TCalculateSizeParams*/ void * AParam)
{
  ...
  {
    const UnicodeString DirPath = base::UnixExtractFilePath(base::UnixExcludeTrailingBackslash(AFile->GetFullFileName()));
  ...
  {
    const int64_t Size = AParams->Size - PrevSize;
          ...
          IsRealFile(File->FileName))
      {
        ...
        if (SubFiles != nullptr)
        {
          ...
          __finally
          {
            if (FirstLevel)
            {
              AOperationProgress->Finish(File->FileName, Success, OnceDoneOperation);
            }
          } end_try__finally
        }
      }
  ...
  const UnicodeString & FileName, const TRemoteFile * File, const UnicodeString & NewName, bool Move, bool DontOverwrite)
{
  ...
  const bool IsBatchOperation = (OperationProgress != nullptr) && (OperationProgress->Operation == foRemoteMove);
...
bool TTerminal::DoMoveFile(const UnicodeString & FileName, const TRemoteFile * File, /*const TMoveFileParams*/ void * Param)
{
  ...
  const bool Result = DoRenameFile(FileName, File, NewName, true, Params.DontOverwrite);
  if (Result)
  {
...
void TTerminal::MoveFile(const UnicodeString & FileName, const TRemoteFile * File, /*const TMoveFileParams*/ void * Param)
{
  DoMoveFile(FileName, File, Param);
}

bool TTerminal::MoveFiles(
  TStrings * AFileList, const UnicodeString & ATarget, const UnicodeString & AFileMask, bool DontOverwrite)
{
  ...
  try__finally
  {
    Result = ProcessFiles(AFileList, foRemoteMove, nb::bind(&TTerminal::MoveFile, this), &Params);
  }
  __finally
  {
    if (GetActive())
...
{
  const bool IsBatchOperation = DebugAlwaysTrue(OperationProgress != nullptr);
  const bool Move = false; // not used
  ...
  TStrings * AFileList, const UnicodeString & ATarget, const UnicodeString & AFileMask, bool DontOverwrite)
{
  ...
  Params.DontOverwrite = DontOverwrite;
  DirectoryModified(ATarget, true);
  return ProcessFiles(AFileList, foRemoteCopy, nb::bind(&TTerminal::CopyFile, this), &Params);
}

void TTerminal::CreateDirectory(const UnicodeString & ADirName, const TRemoteProperties * Properties)
{
  DebugAssert(FFileSystem);
  ...
  const bool Encrypt = Properties->Valid.Contains(vpEncrypt) && Properties->Encrypt;
...
void TTerminal::ChangeDirectory(const UnicodeString & ADirectory)
{
  const UnicodeString DirectoryNormalized = base::ToUnixPath(ADirectory);
  DebugAssert(FFileSystem);
  try
  {
...
bool TTerminal::DoCreateLocalFile(const UnicodeString & AFileName,
  ...
  bool NoConfirmation)
{
  DebugAssert(AOperationProgress);
  DebugAssert(AHandle);
  bool Result = true;
  bool Done = false;
  const DWORD DesiredAccess = GENERIC_WRITE;
  const DWORD ShareMode = FILE_SHARE_READ;
  const DWORD CreationDisposition = CREATE_ALWAYS; // Resume ? OPEN_ALWAYS : CREATE_ALWAYS;
  DWORD FlagsAndAttributes = FILE_ATTRIBUTE_NORMAL;
  do
  {
    *AHandle = this->CreateLocalFile(ApiPath(AFileName),
    ...
    if (!Done)
    {
        ...
        ((LocalFileAttrs & (faReadOnly | faHidden)) != 0))
      {
        if (FLAGSET(LocalFileAttrs, faReadOnly))
        {
          ...
          try__finally
          {
            if (AOperationProgress->GetBatchOverwrite() == boNone)
            {
              Result = false;
            }
            else if ((AOperationProgress->GetBatchOverwrite() != boAll) && !NoConfirmation)
            {
              uint32_t Answer;
              ...
              {
              case qaYesToAll:
                AOperationProgress->SetBatchOverwrite(boAll);
                break;
              case qaCancel:
                AOperationProgress->SetCancel(csCancel); // continue on next case
                Result = false;
                break;
              case qaNoToAll:
                AOperationProgress->SetBatchOverwrite(boNone);
                Result = false;
                break;
              case qaNo:
                Result = false;
        ...
        if (Result)
        {
          ...
          FILE_OPERATION_LOOP_END(FMTLOAD(CANT_SET_ATTRS, AFileName));
        }
        else
        {
          Done = true;
...
bool TTerminal::CreateLocalFile(const UnicodeString & ATargetFileName,
  TFileOperationProgressType * AOperationProgress, HANDLE * AHandle,
  bool NoConfirmation)
{
  DebugAssert(AOperationProgress);
  ...
  FILE_OPERATION_LOOP_BEGIN
  {
    Result = DoCreateLocalFile(ATargetFileName, AOperationProgress, AHandle, NoConfirmation);
  }
  FILE_OPERATION_LOOP_END(FMTLOAD(CREATE_FILE_ERROR, ATargetFileName));
  ...
  bool TryWriteReadOnly)
{
  ...
  FILE_OPERATION_LOOP_END(FMTLOAD(FILE_NOT_EXISTS, ATargetFileName));
  ...
  if (FLAGCLEAR(LocalFileAttrs, faDirectory) || (AHandle == nullptr))
  {
    ...
    FILE_OPERATION_LOOP_BEGIN
    {
      const DWORD Flags = FLAGMASK(FLAGSET(LocalFileAttrs, faDirectory), FILE_FLAG_BACKUP_SEMANTICS);
      LocalFileHandle = this->CreateLocalFile(ApiPath(ATargetFileName), Access,
    ...
    FILE_OPERATION_LOOP_END(FMTLOAD(OPENFILE_ERROR, ATargetFileName));
    ...
    try
    {
      if (AATime || AMTime || ACTime)
      {
        ...
        FILE_OPERATION_LOOP_END(FMTLOAD(CANT_GET_ATTRS, ATargetFileName));
  ...
  const UnicodeString & FileName, const TSearchRecSmart & SearchRec, const TCopyParamType * CopyParam, bool DisallowTemporaryTransferFiles)
{
  ...
  const UnicodeString BaseFileName = GetBaseFileName(FileName);
  ...
  const TRemoteFile * File, const TCopyParamType * CopyParam, bool DisallowTemporaryTransferFiles)
{
  ...
  const UnicodeString FullRemoteFileName = base::UnixExcludeTrailingBackslash(File->FullFileName);
  const UnicodeString BaseFileName = GetBaseFileName(FullRemoteFileName);
  return
  ...
  if (Log->GetLogging() || !CopyParam->AllowAnyTransfer())
  {
    ...
    if (SearchRec == nullptr)
    {
      ...
      else
      {
        ...
        FILE_OPERATION_LOOP_END(FMTLOAD(FILE_NOT_EXISTS, AFileName));
      }
    ...
    {
      AOperationProgress->AddSkippedFileSize(SearchRec->Size);
      Result = false;
    }
  ...
  else
  {
    try
    {
          ...
          DoAllowLocalFileTransfer(AFileName, Rec, Params->CopyParam, false))
      {
        ...
        if (Params->Files != nullptr)
        {
          const UnicodeString FullFileName = ::ExpandUNCFileName(AFileName);
    ...
    for (int32_t Index = 0; Params.Result && (Index < AFileList->GetCount()); ++Index)
    {
      ...
      if (FileSearchRec(FileName, Rec))
      {
        ...
        else
        {
          ...
          const int64_t PrevSize = Params.Size;

          CalculateLocalFileSize(FileName, Rec, &Params);

          if (CalculatedSizes != nullptr)
          {
            const int64_t Size2 = Params.Size - PrevSize;
            CalculatedSizes->push_back(Size2);
          }
...
bool TTerminal::LocalFindFirstLoop(const UnicodeString & APath, TSearchRecChecked & SearchRec)
{
  ...
  FILE_OPERATION_LOOP_END(FMTLOAD(LIST_DIR_ERROR, APath));
  return Result;
}
  ...
  int32_t AFlags, TSynchronizeChecklist * Checklist)
{
  ...
  try__finally
  {
    ...
    if (LocalFindFirstLoop(Data.LocalDirectory + L"*.*", SearchRec))
    {
      ...
      const bool Cached = FLAGSET(AParams, spUseCache) && GetSessionData()->GetCacheDirectories() &&
...
{
  const UnicodeString DefaultAlg = Sha256ChecksumAlg;
  UnicodeString Algs =
  ...
  {
    const UnicodeString A = CutToChar(Algs, L',', true);
  ...
  const UnicodeString RemoteChecksum = FCollectedCalculatedChecksum;
  FCollectedCalculatedChecksum = EmptyStr;
  ...
  const TRemoteFile * AFile, /*TSynchronizeData*/ void * Param)
{
        ...
        Data->Options->MatchesFilter(LocalFileName)))
  {
    ...
    try__finally
    {
      ...
      {
        const int32_t LocalIndex = Data->LocalFileList->IndexOf(LocalFileName);
        New = (LocalIndex < 0);
        if (!New)
        {
  ...
  const TRemoteFile * AFile, void * /*Param*/)
{
  ...
  {
    HANDLE Handle;
    this->OpenLocalFile(LocalFile, GENERIC_WRITE, nullptr, &Handle,
...
TStrings * TTerminal::GetShellChecksumAlgDefs()
{
  if (FShellChecksumAlgDefs == nullptr)
  {
    ...
    if (ChecksumCommandsDef.IsEmpty())
    {
      ...
      AddToList(ChecksumCommandsDef, UnicodeString(Md5ChecksumAlg) + L"=md5sum", Delimiter);
    }
...
int32_t TTerminal::CopyToParallel(TParallelOperation * ParallelOperation, TFileOperationProgressType * AOperationProgress)
{
  UnicodeString FileName;
  ...
  const int32_t Result = ParallelOperation->GetNext(this, FileName, Object, TargetDir, Dir, Recursed, CustomCopyParam);
  ...
  if (Result > 0)
  {
    ...
    const int32_t Prev = AOperationProgress->GetFilesFinishedSuccessfully();
    DebugAssert((FOperationProgress == AOperationProgress) || (FOperationProgress == nullptr));
    ...
    try__finally
    {
      ...
      if (ParallelOperation->GetSide() == osLocal)
      {
        FFileSystem->CopyToRemote(
          FilesToCopy.get(), TargetDir, CopyParam, Params, AOperationProgress, OnceDoneOperation);
      }
...
void TTerminal::CopyParallel(TParallelOperation * ParallelOperation, TFileOperationProgressType * AOperationProgress)
{
  try__finally
  {
    bool Continue = true;
    do
    {
      const int32_t GotNext = CopyToParallel(ParallelOperation, AOperationProgress);
      if (GotNext < 0)
      {
        Continue = false;
      }
  ...
  TFileOperationProgressType * AOperationProgress, bool Parallel, TStrings * AFiles)
{
  if (FLog->GetLogging())
  {
    const UnicodeString TargetSide = (AOperationProgress->GetSide() == osLocal) ? L"remote" : L"local";
    UnicodeString S =
      FORMAT(
        "Copying %d files/directories to %s directory \"%s\"",
        AOperationProgress->GetCount(), TargetSide, ATargetDir);
    ...
    if (AOperationProgress->GetTotalSizeSet())
    {
      S += FORMAT(" - total size: %s", FormatSize(AOperationProgress->GetTotalSize()));
    }
    LogEvent(S);
...
void TTerminal::LogTotalTransferDone(TFileOperationProgressType * AOperationProgress)
{
  LogEvent(L"Copying finished: " + AOperationProgress->GetLogStr(true));
}
  ...
  TParallelOperation * ParallelOperation)
{
  ...
  try
  {
    ...
    {
      const bool CalculateSize = ACanParallel || CopyParam->CalculateSize;
    ...
    try__finally
    {
      ...
      try__finally
      {
        const bool Parallel = ACanParallel && CalculatedSize;
...
{
  DebugAssert((AFilesToCopy != nullptr) && (AOperationProgress != nullptr));
    ...
    try__finally
    {
      try
      {
        if (GetSessionData()->GetCacheDirectories())
        {
          ...
          {
            const UnicodeString FileNameOnly = base::ExtractFileName(FileName, false);
            DirectoryModified(FullTargetDir + FileNameOnly, true);
          }
        }
        SourceRobust(FileName, SearchRec, FullTargetDir, CopyParam, AParams, AOperationProgress, AFlags | tfFirstLevel);
        Success = true;
      }
      catch(ESkipFile & E)
      {
  ...
  TFileOperationProgressType * AOperationProgress, uint32_t AFlags)
{
  ...
  do
  {
    ...
    {
      AOperationProgress->RollbackTransfer();
      Action.Restart();
  ...
  const UnicodeString & ADirectoryPath, uint32_t Attrs, const TCopyParamType * CopyParam)
{
  ...
  if (DoCreate)
  {
    ...
    CreateDirectory(ADirectoryPath, &Properties);
  }
  return DoCreate;
}
  ...
  TFileOperationProgressType * AOperationProgress, uint32_t AFlags)
{
  ...
  const bool DoRecurse = FLAGCLEAR(AParams, cpNoRecurse);
  if (DoRecurse)
  {
    TSearchRecOwned SearchRec;
    bool FindOK = LocalFindFirstLoop(ADirectoryName + L"*.*", SearchRec);
    while (FindOK && !AOperationProgress->Cancel)
    {
      ...
      catch(ESkipFile & E)
      {
        ...
        TSuspendFileOperationProgress Suspend(AOperationProgress);
  ...
  if (DoRecurse && !AOperationProgress->GetCancel())
  {
...
{
  const bool AsciiTransfer = UseAsciiTransfer(ABaseFileName, Side, CopyParam, MaskParams);
  OperationProgress->SetAsciiTransfer(AsciiTransfer);
  const UnicodeString ModeName = OperationProgress->AsciiTransfer ? L"Ascii" : L"Binary";
...
void TTerminal::SelectSourceTransferMode(const TLocalFileHandle & Handle, const TCopyParamType * CopyParam)
{
  ...
  const UnicodeString BaseFileName = GetBaseFileName(Handle.FileName);
...
void TTerminal::DoDeleteLocalFile(const UnicodeString & FileName)
{
  ...
  FILE_OPERATION_LOOP_END(FMTLOAD(CORE_DELETE_LOCAL_FILE_ERROR, FileName));
}
  ...
  TFileOperationProgressType * AOperationProgress, uint32_t AFlags, TUploadSessionAction & Action, bool & ChildError)
{
  ...
  AOperationProgress->SetFile(AFileName, false);

  if (!AllowLocalFileTransfer(AFileName, SearchRec, CopyParam, AOperationProgress))
  {
    throw ESkipFile();
  }
  ...
  if (Handle.Directory)
  {
    ...
    DirectorySource(
      ...
      CopyParam, AParams, AOperationProgress, AFlags);
  }
  else
  {
    if (!CopyParam->OnTransferIn.empty())
    ...
    else
    {
      ...
      AOperationProgress->SetLocalSize(Handle.Size);

      // Suppose same data size to transfer as to read
      // (not true with ASCII transfer)
      AOperationProgress->SetTransferSize(AOperationProgress->GetLocalSize());
    }
  ...
  UnicodeString & ParallelFileName, int64_t & ParallelFileSize, TFileOperationProgressType * AOperationProgress)
{
      ...
      FFileSystem->IsCapable(fcParallelFileTransfers))
  {
    ...
    if (TParallelOperation::GetOnlyFile(Files, ParallelFileName, ParallelObject))
    {
      ...
      {
        const UnicodeString BaseFileName = GetBaseFileName(ParallelFileName);
        TFileMasks::TParams MaskParams;
          ...
          if (base::FileExists(ApiPath(DestFullName)))
          {
            ...
            const uint32_t Answer =
              ConfirmFileOverwrite(
                ...
                CopyParam, Params, AOperationProgress, EmptyStr);
            switch (Answer)
            {
              case qaCancel:
              case qaNo:
  ...
  try__finally
  {
    ...
    try__finally
    {
      ...
      try
      {
        try__finally
        {
          const bool Parallel = ACanParallel && TotalSizeKnown;
          LogTotalTransferDetails(ATargetDir, CopyParam, &OperationProgress, Parallel, Files.get());
    ...
    __finally
    {
      if (CollectingUsage)
      {
        const int32_t CounterTime = TimeToSeconds(OperationProgress.TimeElapsed());
  ...
  TFileOperationProgressType * AOperationProgress, uint32_t AFlags, TOnceDoneOperation & OnceDoneOperation)
{
  ...
  const UnicodeString FullTargetDir = IncludeTrailingBackslash(ATargetDir);
  int32_t Index = 0;
  while ((Index < AFilesToCopy->GetCount()) && !AOperationProgress->GetCancel())
  {
    bool Success = false;
    ...
    __finally
    {
      OperationFinish(AOperationProgress, File, FileName, Success, OnceDoneOperation);
    } end_try__finally
    Index++;
  }
}
  ...
  try__finally
  {
    ...
    do
    {
      try
      {
        ...
        if (!Sunk)
        {
          Sink(AFileName, AFile, ATargetDir, CopyParam, AParams, AOperationProgress, AFlags, Action);
          Sunk = true;
        }
        ...
        if (FLAGSET(AParams, cpDelete))
        {
          ...
          DeleteFile(AFileName, AFile, &Params);
        }
      }
      catch(Exception & E)
      {
  ...
  TDownloadSessionAction & Action)
{
  ...
  {
    AOperationProgress->AddSkippedFileSize(AFile->Resolve()->GetSize());
    throw ESkipFile();
  }

  LogFileDetails(AFileName, AFile->GetModification(), AFile->GetSize(), AFile->LinkedFile);

  AOperationProgress->SetFile(AFileName);
  ...
  const UnicodeString DestFullName = ATargetDir + DestFileName;
  ...
  if (AFile->GetIsDirectory())
  {
    ...
    if (CanRecurseToDirectory(AFile))
    {
      ...
      if (FLAGCLEAR(AParams, cpNoRecurse))
      {
        ...
        SinkFileParams.OperationProgress = AOperationProgress;
  ...
  else
  {
    ...
    // Will we use ASCII or BINARY file transfer?
    const UnicodeString BaseFileName = GetBaseFileName(AFileName);
    ...
    AOperationProgress->SetTransferSize(TransferSize);
    ...
    FFileSystem->Sink(
      AFileName, AFile, ATargetDir, DestFileName, Attrs, CopyParam, AParams, AOperationProgress, AFlags, Action);

    LogFileDone(AOperationProgress, LogFileName, Action);
    AOperationProgress->Succeeded();
  }
}
  ...
  TSessionInfo & SessionInfo)
{
  ...
  else
  {
    ...
    const bool WindowsValidated = NeonWindowsValidateCertificateWithMessage(Data, WindowsValidatedMessage);

    const UnicodeString SiteKey = TSessionData::FormatSiteKey(AHostName, APortNumber);
    Result =
      VerifyCertificate(
...
{
  const UnicodeString FileDir = base::UnixExtractFileDir(APath);
...
{
  // can be optimized
  const bool Result = (EncryptFileName(APath, EncryptNewFiles) != APath);
  return Result;
}
...
UnicodeString TTerminal::DecryptFileName(const UnicodeString & Path, bool DecryptFullPath, bool DontCache)
{
  ...
  if (IsEncryptingFiles() && !base::IsUnixRootPath(Path))
  {
    ...
    const UnicodeString FileNameEncrypted = FileName;

    const bool Encrypted = TEncryption::IsEncryptedFileName(FileName);
    if (Encrypted)
    {
      TEncryption Encryption(FEncryptKey);
...
UnicodeString TTerminal::UploadPublicKey(const UnicodeString & FileName)
{
  ...
  const bool PrevAutoReadDirectory = FAutoReadDirectory;
  const bool PrevExceptionOnFail = ExceptionOnFail;

  const UnicodeString AuthorizedKeysFilePath = FORMAT(L"%s/%s", OpensshFolderName, OpensshAuthorizedKeysFileName);
  ...
  const FILETIME * AcTime, const FILETIME * WrTime)
{
  ...
  FILE_OPERATION_LOOP_BEGIN
  {
    ...
    try__finally
    {
      this->OpenLocalFile(LocalFileName, GENERIC_WRITE, nullptr,
